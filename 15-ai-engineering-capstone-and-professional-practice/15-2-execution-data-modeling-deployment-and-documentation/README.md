# 15.2 Execution: Data, Modeling, Deployment & Documentation

Provides a practical operating system for delivering a capstone from data and modeling through deployment and reproducible documentation.

## Start Here

1. Theory: `theory/15-2-execution-data-modeling-deployment-and-documentation.md`
2. Notebook: `notebooks/15-2-capstone-execution-checklists-and-skeleton.ipynb`

## Outcomes

- Apply end-to-end AI engineering checklists in a scoped project.
- Choose realistic deployment patterns for capstone constraints.
- Produce interview-ready technical artefacts and decision records.

## Includes

- execution sprint template,
- deployment and documentation scoring rubric,
- evidence package checklist for interviews and portfolio reviews,
- reusable skeletons for data, modeling, serving, and runbooks.
